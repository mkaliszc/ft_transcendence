...
onMounted(() => {
  ...
  const handleSignIn = async () => {
	...
	try {
	  ...
	  if (form.value.email === 'demo@example.com' && form.value.password === 'demo123') {
		const userData = {
		  id: 1,
		...
		// Naviguer vers le jeu au lieu d'émettre un événement
		router.push('/game')
	  } else {
		throw new Error(t('invalidCredentials'))
	  }
	} catch (err) {
	  error.value = err.message || t('connectionError')
	} finally {
	  loading.value = false
	}
  }
