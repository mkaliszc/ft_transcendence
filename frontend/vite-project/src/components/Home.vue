<template>
  <div class="home-container">
    ...
    <main class="hero-section">
      ...
      <!-- Boutons de fonctionnalités (cliquables) -->
...
const initialLocale =
  savedLanguage && ["en", "fr", "es"].includes(savedLanguage)
    ? savedLanguage
    : null;
if (initialLocale) {
...
const animatePong = () => {
  ...
  if (ballX.value >= 380) {
    if (
      ballY.value >= rightPaddleY.value &&
      ballY.value <= rightPaddleY.value + 60
    ) {
      ballSpeedX.value = -ballSpeedX.value;
    } else {
      // Reset la balle
...
// Nouveaux navigateurs pour les fonctionnalités
const goToFeature1 = () => {
...
const goToFeature3 = () => {
  router.push({ name: "Tournaments" }); // Remplacez "Tournaments" par le nom exact de la route si elle existe
};
