<template>
  <!-- Garder le même template que vous avez fourni -->
...
// Méthode d'inscription modifiée pour utiliser l'API
const handleSignUp = async () => {
  ...
  try {
    ...
    if (response.token) {
      ...
      emit('signup-success', response.user || {
        id: response.userId,
        email: form.value.email,
        firstName: form.value.firstName,
        lastName: form.value.lastName
    ...
    if (err.response) {
      ...
      if (status === 409) {
        ...
        error.value = t('emailAlreadyExists')
      } else if (status === 400) {
        // Erreur de validation
        error.value = data.message || t('validationError')
      } else {
        // Autre erreur
        error.value = data.message || t('signupError')
      }
    } else if (err.request) {
      // Pas de réponse du serveur
      error.value = t('serverNotResponding')
    } else {
...
const handleGoogleSignUp = async () => {
  // Vous devrez implémenter l'authentification Google ici
  // Pour l'instant, nous affichons juste une alerte
  alert(t('googleSignUpNotImplemented'))
  
  // Exemple d'implémentation avec vue3-google-oauth2
  /*
  try {
    ...
    error.value = t('googleSignUpError')
  }
  */
}
