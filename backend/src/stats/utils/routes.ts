...
export default fp(async function routes_stats(fastify: any) {
	fastify.get('/stats/user/:username', { preHandler: fastify.authenticate }, getUser);
	...
	fastify.get('/stats/history/:username', { preHandler: fastify.authenticate }, getHistory);
	fastify.patch('/stats/friendRequest', { preHandler: fastify.authenticate }, friendrequest); 
